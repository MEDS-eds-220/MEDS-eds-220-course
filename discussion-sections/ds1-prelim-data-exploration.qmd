...
## General directions
:::{.callout-tip appearance="minimal"}
-  To helps everyone talk through ideas, catch errors early, and learn multiple ways to solve a problem, we will be working in our teams each week. There are two different roles you might have during discussion section: the driver or the navigator. There will be one driver and three navigators each week. The driver will plug their computer into the tv at your table and be responsible for writing the code that your team comes up with (note: you all must write the code individually as well, but the person coding on the tv will be typing on the notebook that the whole group works on together). The rest of the team, the navigators, will  read the prompts, spots errors, asks “what if” questions. Navigators should be asking questions such as the following:
