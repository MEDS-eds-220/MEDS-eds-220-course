...
If you already have a PAT that you can copy paste, you can skip to step 2. Otherwise, create one. You can follow the instructions in the [MEDS installation guide](https://ucsb-meds.github.io/MEDS-installation-guide/#GitHub-PAT) or create one directly through GitHub.


## 2. Set up the git credential helper

a. Using the terminal, navigate to the directory from where you want to push your updates. Make sure you are on that directroy (for example, the `eds-2025-in-class` by running `pwd` on the terminal. 
...
c. Run `more .gitconfig` on the terminal. You should see something like this as the output:

```default
[user]
        name = carmengg
        email = c_galazgarcia@ucsb.edu
[credential]
        helper = store
```
...
a. Make a small update to one of your notebooks and save the file. 

b. Go through the steps to make a push. Git will prompt you for your credentials, use your GitHub username and use the PAT as the password. Press enter and finalize the push!

c. Run `more. git-credentials` on the terminal. You should see something like this as the output:
