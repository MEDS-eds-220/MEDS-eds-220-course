...
::: {.callout-caution}
## Library or package?
A **package** in Python refers to a specific way of organizing multiple files of code into a directory hierarchy, often within a large code library. The words "library" and "package" are often used interchangeably. NumPy, for example, is both a library and a package. 
:::

Let's start by importing the NumPy library by using the standard abbreviation, `np`:
...
There are many ways of constructing multi-word variable names. In this course we will name variables using **snake_case**, where words are all lowercase and separated by underscores (ex: `raw_data`, `fires_2023`). This is the naming convention suggested by the [PEP 8 - Style Guide for Python Code](https://peps.python.org/pep-0008/#function-and-variable-names) [@pep8]. **Remember variable names should be both descriptive and concise!**
:::

## Objects
You will often encounter the word **object** in Python documentation and tutorials. Informally speaking, an object is a bundle of *properties* and *actions* about something specific. For example, an object could represent a data frame with properties such as number of rows, names of columns, and date created, and actions such as selecting a specific row or adding a new column. 
...
The NumPy type `numpy.int64` is telling us 1 is an integer stored as a 64-bit number. NumPy has its own data types to deal with numbers depending on memory storage and floating point precision, [click here to see all the types](https://numpy.org/doc/stable/reference/arrays.scalars.html#sized-aliases). 
...
You can see there are different types of arguments inside the parentheses. Roughly speaking, a function has two types of arguments:
...
`end` is a parameter in `print` with the default value a new line. We can pass the value ` ^_^` to this parameter so that it finishes the line with ` ^_^` instead:
...
What kind of parameter is `axis` in our previous call of the `min` method?
:::
...
|package | attribute | output | parameter(s) | optional|
