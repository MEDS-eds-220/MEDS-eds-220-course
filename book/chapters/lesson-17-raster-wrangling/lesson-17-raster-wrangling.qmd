...
Notice a warning appeared when we clipped the raster. After some investigation, we would have found that this `RuntimeWarning` occurs because the `rio.clip` function cannot fill in the region clipped regions with To make sure the clipping operator fills in any pixels with the adequate 

```{python}
(lulc.rio.clip_box(*thomas_fire_match.total_bounds)
