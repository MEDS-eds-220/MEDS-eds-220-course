...
col_index = penguins.columns.get_loc('bill_length_mm')
penguins.iat[3,col_index] = np.nan
...
- `new_values` is the new value or values we want. If using multiple values, then `new_values` must be of the same length as the number of rows selected. Using `loc[]` in assignment modifies the data frame directly without the need for reassignment.
