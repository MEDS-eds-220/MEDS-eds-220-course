...
In this example, notice that the `axes` variable returned by the `plt.subplots()` function is actually a list of axes we can iterate over. Remember that the figure and the axes are separete elements in a `matplotlib` plot.
...
Let's create a function that will give us the top 3 primary energy sources for a given state:

```{python}
def top3_energy_sources(state, power_plants):
    """
...
for state in ['New Jersey', 'New York', 'Pennsylvania']:
    print(state, ': ', top3_energy_sources(state, power_plants))
```
...
```{python}
def plot_top3_energy_sources(state, sources, power_plants):
    
    if len(sources) != 3:
        print('sources must have three elements to produce the plot')
        return
...
```{python}
plot_top3_energy_sources('New Jersey', 
                         top3_energy_sources('New Jersey', power_plants),
                         power_plants)
```
