...
- objective 2
- objective 3
- Understand what it means to perform an operation on a `pandas.DataFrame` in-place 
- Use the `info()`, `describe()`, `nunique()`, `unique()` and `value_counts()` methods to obtain preliminary information about a `pd.DataFrame`.
...
3. Run the code and examine the graph. Do we have all the necessary information to make sure it make sense to directly compare the surveys at these different sites? 
