...
A series is a *one-dimensional* array of *indexed* data. A `pandas.Series` having an **index** is the main difference between a `pandas.Series` and a NumPy array. Let's see the difference:
...
Notice the index is printed as part of the `pandas.Series` while, although the `np.array` is indexable, the index is not part of this data structure.

![Printing the `pands.Series` shows the index, values, and data type of the values.](/book/images/lesson-2/series-parts.png){width=80%}
...
In `pandas` we can represent a missing or NA value with the float value `numpy.nan`, which stands for "not a number". Let's construct a small series with some NA values represented this way:
...
The [`hasnans` attribute](https://pandas.pydata.org/docs/reference/api/pandas.Series.hasnans.html) for a `pandas.Series` returns `True` if there are any `np.nan` values in it and false otherwise:
...
After detecting there are `np.nan` values, we might be intersted in knowing which elements in the series are NAs. We can do this using the [`isna` method](https://pandas.pydata.org/docs/reference/api/pandas.Series.isna.html):

```{python}
s.isna()
```

The ouput is a `pandas.Series` of boolean values indicating if an element in the row at the given index is `np.nan` (`True` = is `np.nan`) or not (`False` = not `np.nan`).

:::{.callout-tip}
## Check-In
Create a small series then use duplicated.

[`pandas` documentation](https://pandas.pydata.org/docs/reference/api/pandas.Series.html)
:::
