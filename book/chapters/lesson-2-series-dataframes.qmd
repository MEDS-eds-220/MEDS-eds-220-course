...
- Construct simple `pandas.Series` and `pandas.DataFrame` from scratch using different initialization methods 
...
Always import all your packages in a single cell at the top of your notebook! Following the [PEP 8 - Style Guide for Python Code](https://peps.python.org/pep-0008/#imports) @pep8, each package or library import should be in a separate line. 
:::
<!--
There is so much to learn about `pandas`. While we won't be able to cover every single functionality of this package in the next three lectures, the goal is to get you started with the basic tools for data wrangling and give you a solid basis on which you can explore further. 
-->
## Series
...
After detecting there are NA values, we might be interested in knowing which elements in the series are NAs. We can do this using the [`isna` method](https://pandas.pydata.org/docs/reference/api/pandas.Series.isna.html):

```{python}
s.isna()
```

The output is a `pandas.Series` of boolean values indicating if an element in the row at the given index is `np.nan` (`True` = is NA) or not (`False` = not NA).
...
# slicing will return elements at indices 2 through 4 (inclusive)
x[2:5]
```
